...
class Frontend_Assets {
	...
	public function __construct() {
		...
		add_action( 'wp_head', array( $this, 'inject_fetch_priority' ), 1 );
	}

	/**
	 * Inject fetch priority high to header
	 */
	public function inject_fetch_priority() {
		if ( is_singular() && have_posts() ) {
			$post = get_post();
			if ( has_blocks( $post->post_content ) ) {
				$blocks = parse_blocks( $post->post_content );
				$this->find_and_inject_fetch_priority( $blocks );
			}
		}
	...
	/**
	 * Recursive find and inject fetch priority
	 *
	 * @param array $blocks Blocks.
	 */
	private function find_and_inject_fetch_priority( $blocks ) {
		foreach ( $blocks as $block ) {
			if ( ! empty( $block['attrs']['background'] ) ) {
				$bg        = $block['attrs']['background'];
				$image_url = '';

				if ( ! empty( $bg['useFeaturedImage'] ) && has_post_thumbnail() ) {
					$use_featured = $bg['useFeaturedImage'];
					if ( is_array( $use_featured ) && ( ! empty( $use_featured['Desktop'] ) || ! empty( $use_featured['Tablet'] ) || ! empty( $use_featured['Mobile'] ) ) ) {
			...
			if ( ! empty( $block['innerBlocks'] ) ) {
				$this->find_and_inject_fetch_priority( $block['innerBlocks'] );
			}
		}
	}
