...
const generateChart = (chartData, element) => {
    const {
        cutout,
        minValue,
        chartType,
    ...
    const svgContainer = select(element)
        .append('svg')
        .attr('width', width)
        .attr('height', height);
...
const appendGradientDefs = (svg, data, elementId, type) => {
    ...
    data.forEach((d, i) => {
        ...
        if (d.gutenverseGradient) {
            ...
            gradient.append('stop')
                ...
                .attr('stop-color', theColor(d.colorGradientOne));

            gradient.append('stop')
                .attr('offset', '100%')
                .attr('stop-color', theColor(d.colorGradientTwo));
        }
    });
};

const drawDonutChart = (props) => {
    const {
        svg,
        radius,
        cutout,
    ...
    appendGradientDefs(svg, data, elementId, 'donut');
    ...
    svg.append('path')
        ...
        .attr('fill', theColor(cutoutBackground))
        .attr('stroke', 'none');
    ...
    const arcs = svg.selectAll('path.donut')
        ...
        .attr('fill', (d, i) =>
            d.data.colorMode === 'gradient'
                ...
                : theColor(d.data.backgroundColor)
        )
...
const getMaxvalue = (data, totalValue, chartContent) => {
    if(chartContent !== 'number') {
...
const drawBarChart = (props) => {
    ...
    appendGradientDefs(svg, data, elementId, 'bar');

    const margin = { top: 20, right: 20, bottom: 40, left: 40 };
    const chartWidth = width - margin.left - margin.right;
    const chartHeight = height - margin.top - margin.bottom;
    const topValue = getMaxvalue(data, totalValue, chartContent);
    ...
    const bars = chartGroup.selectAll('rect')
        ...
        .attr('fill', (d, i) =>
            d.colorMode === 'gradient'
                ? `url(#gradient-${elementId}-${i})`
                : theColor(d.backgroundColor)
        )
        .style('stroke', d => theColor(d.borderColor))
    ...
    legendDisplay && appendLegend({ svgContainer, svg, data, position: 'top', elementId, height });
};

const appendLegend = ({ svgContainer, svg, data, position = 'top', elementId, height, radius = 8 }) => {
