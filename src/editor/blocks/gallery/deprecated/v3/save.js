...
const save = compose(
    ...
    const imageCondition = (image) => {
        if(image.lazyLoad){
            return <img className="main-image" src={image.src ? image.src.image : imagePlaceholder} loading="lazy"/>;
        }else{
            return <img className="main-image" src={image.src ? image.src.image : imagePlaceholder}/>;
        }
    };
